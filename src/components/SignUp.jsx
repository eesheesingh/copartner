...
const SignUp = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (mobile.length !== 10) {
      ...
      setShowOtp(true); // Show the OTP popup after form submission
    }
  };
  ...
  const handleClosePopups = () => {
    ...
    // Additional logic to close the sign-up popup if needed
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-[#18181B] border-[1px] border-[#ffffff2a] m-4 p-6 rounded-lg w-96 relative text-center">
        <div className='absolute top-3 right-0 text-right '>
          <div onClick={handleClosePopups} className="text-gray-400 w-8 text-[20px] cursor-pointer hover:text-white">
            <IoClose />
          </div>
        </div>
        <div className="mb-4">
